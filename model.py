...
class LanguageModel(Model):  # Container and Controller
    ...
    def define_lang_interaction(self, ag1, ag2, ret_pcts=False):
        """ Method to find out lang of interaction between two lang agents
            Args:
                * ag1, ag2 : lang agent instances
                *ret_pcts : Boolean. If False, only lang is returned.
                            If True, both lang and agent lang knowledge is returned
        """
    ...
    def step(self):
        self.data_process.datacollector.collect(self)
        self.schedule.step()
    ...
    def run_model(self, steps, recording_steps_period=None, save_dir=''):
        ...
        """
        proc_bar = pyprind.ProgBar(steps)
    ...
    def run_and_animate(self, steps, plot_type='imshow'):
        ...
        ax4.set_xlim(0, self.grid.width-1)
        ax4.set_ylim(0, self.grid.height-1)
        if plot_type == 'imshow':
            im_2D = ax4.imshow(np.zeros((self.grid.width, self.grid.height)),
        ...
        def init_show():
            if plot_type == 'imshow':
                im_2D.set_array(np.random.choice([np.nan, 0], p=[1, 0], size=(self.grid.width, self.grid.height)))
                return im_2D,
            elif plot_type == 'scatter':
        ...
        def run_and_update(i):
            ...
            self.get_agents_attrs_value('language')
            # create 2D plot
...
#
#
# class Language_Model_OLD(Model):
#
#     ic_pct_keys = [10, 25, 50, 75, 90]
#     family_size = 4
#
